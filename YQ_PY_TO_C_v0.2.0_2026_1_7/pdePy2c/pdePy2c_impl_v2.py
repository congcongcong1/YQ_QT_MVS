...
class CCodeGenerator(ast.NodeVisitor):
    ...
    def _parse_2d_list_literal(self, node: ast.List) -> Tuple[int, int, List[str]] | None:
        if not node.elts:
            return None
        ...
        for row in node.elts:
            ...
            if cols is None:
                ...
                raise TranslateError("2D list rows must have the same length")
            for e in row.elts:
                if not isinstance(e, ast.Constant) or not isinstance(e.value, int):
                    raise TranslateError("2D list only supports int constants")
                flat.append(str(e.value))
        if cols is None:
            cols = 0
    ...
    def _parse_namedtuple_def(self, node: ast.Assign) -> Tuple[str, List[str]] | None:
        ...
        if len(node.value.args) < 2:
            raise TranslateError("namedtuple requires (typename, fields)")
    ...
    def _emit_global_assign(self, node: ast.Assign) -> None:
        ...
        if isinstance(val, ast.List):
            ...
            if parsed_2d:
                ...
                self.type_stack[0][var] = "int_arr2d"
    ...
    def _get_expr_type(self, node: ast.AST) -> str:
        ...
        if isinstance(node, ast.List):
            if self._parse_2d_list_literal(node):
                return "int_arr2d"
            return "int_arr"
        ...
        if isinstance(node, ast.Call):
            ...
            if isinstance(node.func, ast.Name):
                if node.func.id in self.struct_names:
                    return node.func.id
                if node.func.id == "width":
                    return "int"
    ...
    def emit_print_call(self, node: ast.Call):
        ...
        if len(args) == 2 and isinstance(args[0], ast.Constant) and isinstance(args[0].value, str):
            ...
            # Case: print("...", arr) -> 打印数组
            if isinstance(a1, ast.Name) and self.is_array2d(a1.id):
                h = self.get_len_name(a1.id)
                w = self.get_width_name(a1.id)
                c_name = self.get_renamed_param(a1.id)
                if not h or not w:
                    raise TranslateError(f"2D array {a1.id} missing size metadata")
                ...
                self.emit(f'printf("%d", {c_name}[{iy} * {w} + {ix}]);')
                self.dedent()
                self.emit('}')
                self.emit('printf("]");')
                self.dedent()
            ...
            if ln:
                ...
                self.indent(); self.emit(f'printf("%d", {c_name}[{idx}]);')
        ...
        for idx_arg, a in enumerate(args):
            ...
            # 数组打印 (带方括号格式)
            if isinstance(a, ast.Name) and self.is_array2d(a.id):
                h = self.get_len_name(a.id)
                w = self.get_width_name(a.id)
                c_name = self.get_renamed_param(a.id)
                if not h or not w:
                    raise TranslateError(f"2D array {a.id} missing size metadata")
                iy = self.fresh_var("__y_print")
                ix = self.fresh_var("__x_print")
                self.emit('printf("[");')
                self.emit(f'for (int {iy} = 0; {iy} < {h}; {iy}++) {{')
            ...
            if ln:
                ...
                self.emit(f'printf("%d", {c_name}[{idx}]);')
                self.dedent()
                self.emit('}')
                if last:
                    self.emit('printf("]\\n");')
    ...
    def generate(self, tree: ast.AST) -> str:
        ...
        self.emit("")
        self.emit("static char *pyc_str_concat(const char *a, const char *b) {")
        self.indent()
        self.emit("size_t la = strlen(a);")
        self.emit("size_t lb = strlen(b);")
        self.emit("char *out = (char *)malloc(la + lb + 1);")
        self.emit("if (!out) return NULL;")
        self.emit("memcpy(out, a, la);")
        self.emit("memcpy(out + la, b, lb);")
        self.emit("out[la + lb] = '\\0';")
    ...
    def visit_Module(self, node: ast.Module):
        ...
        for i, s in enumerate(node.body):
            if isinstance(s, ast.ClassDef):
                fields = self._parse_class_struct(s)
        ...
        funcs = [s for i, s in enumerate(node.body) if isinstance(s, ast.FunctionDef) and i not in struct_nodes]
        # 预计算返回类型与 main 重命名映射
        for f in funcs:
        ...
        if isinstance(node.value, ast.Name):
            ...
            if t in self.struct_names:
                if node.attr not in self.struct_defs.get(t, []):
                    raise TranslateError(f"struct {t} 没有字段 {node.attr}")
    ...
    def visit_Assign(self, node: ast.Assign):
        ...
        if isinstance(target, ast.Attribute) and isinstance(target.value, ast.Name):
            ...
            if target.attr not in self.struct_defs.get(t, []):
                raise TranslateError(f"struct {t} 没有字段 {target.attr}")
        ...
        if isinstance(val_node, ast.List):
            ...
            if parsed_2d:
                ...
                self.set_type_name(var, "int_arr2d")
    ...
    def visit_BinOp(self, node: ast.BinOp) -> str:
        if isinstance(node.op, ast.Add):
            ...
            if lt == "char *" or rt == "char *":
                if lt != "char *" or rt != "char *":
                    raise TranslateError("string concatenation only supports str + str")
                left = self.visit(node.left)
                right = self.visit(node.right)
                return f"pyc_str_concat({left}, {right})"
