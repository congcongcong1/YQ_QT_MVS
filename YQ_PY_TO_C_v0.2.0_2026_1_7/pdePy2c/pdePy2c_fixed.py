...
class CCodeGenerator(ast.NodeVisitor):
    ...
    def _emit_global_assign(self, node: ast.Assign) -> None:
        ...
        if isinstance(val, ast.Constant):
            if isinstance(val.value, float):
                ...
                c_type = "char *"
            elif self._is_uint16_literal(val):
                c_type = "uint16_t"
            else:
                c_type = "int"
    ...
    def emit_print_call(self, node: ast.Call):
        ...
        for idx_arg, a in enumerate(args):
            ...
            if atype == "double":
                ...
                fmt = "%u\\n" if last else "%u"
                self.emit(f'printf("{fmt}", (uint16_t){val_str});')
            else:
                fmt = "%d\\n" if last else "%d"
    ...
    def visit_Constant(self, node: ast.Constant) -> str:
        ...
        if node.value is None:
            return "0"
        if isinstance(node.value, int) and (self._is_hex_literal(node) or self._is_octal_literal(node)):
            segment = self._get_source_segment(node)
            if segment:
                return self._normalize_int_literal(segment)
...
def translate_files(pyfiles: List[str], out_dir: str, out_path: str) -> None:
    ...
    for f in pyfiles:
        try:
            ...
            c_code = CCodeGenerator(filename=f, source_text=src).generate(tree)
