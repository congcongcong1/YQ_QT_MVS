...
public slots:
    // -----------------------------------------------------------------------
        // [最终稳定版] doProcess: 双通道竞争模式 (Double-Channel Competition)
...
private:
    ...
    QVector<float> m_calibRadii;           // <--- 新增：对应的 9 个标定圆的半径
    /**
    ...
    QVector<cv::Point2f> m_circleCenterOverlay;    // <---新增：存储多个圆心坐标
    QVector<float> m_circleRadiusOverlay;          // <---新增：存储多个半径
...
//    float       m_circleRadiusOverlay = 0.0f;//原来单个圆的半径
    ...
    // 请根据你实际打印的纸张修改这两个值！
    const cv::Size m_boardSize = cv::Size(6, 5);
