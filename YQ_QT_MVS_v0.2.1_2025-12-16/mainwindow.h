...
public slots:
    // -----------------------------------------------------------------------
    // [修改版] doProcess: 加入边缘剔除和形状过滤，防止误检背景噪声
    // -----------------------------------------------------------------------
    void doProcess(cv::Mat smallMat, double scale) {
        ...
        // 2. 二值化
        // 注意：如果是全黑背景，OTSU 可能会算出很低的阈值(如 5)，导致噪点变成物体。
        // 这里加一个保护：如果计算出的阈值太低，就强制使用一个最小阈值
        double otsuThresh = cv::threshold(gray, bin, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
        if (otsuThresh < 30) {
            // 如果环境太暗，OTSU 阈值过低，强制用固定阈值过滤噪声
             cv::threshold(gray, bin, 30, 255, cv::THRESH_BINARY);
        }
        ...
        // 获取小图的宽高，用于边缘检测
        int matW = smallMat.cols;
        int matH = smallMat.rows;
        int margin = 5; // 边缘容差像素
        ...
        for (const auto &cnt : contours) {
            ...
            // --- [新增] 过滤逻辑 A：边缘剔除 ---
            // 如果轮廓紧贴图像四个边缘，视为无效（通常是光照不均或传感器边缘噪声）
            cv::Rect box = cv::boundingRect(cnt);
            if (box.x <= margin || box.y <= margin ||
                (box.x + box.width) >= (matW - margin) ||
                (box.y + box.height) >= (matH - margin))
            {
                continue; // 丢弃贴边的轮廓
            }

            // --- [新增] 过滤逻辑 B：形状过滤 (可选) ---
            // 如果物体不是特别细长的线，可以校验长宽比
             cv::RotatedRect rCheck = cv::minAreaRect(cnt);
             float w = rCheck.size.width;
             float h = rCheck.size.height;
             float ratio = (w < h) ? w / h : h / w;
             if (ratio < 0.1) continue; // 如果宽高比小于 1:10 (太细)，视为噪点
