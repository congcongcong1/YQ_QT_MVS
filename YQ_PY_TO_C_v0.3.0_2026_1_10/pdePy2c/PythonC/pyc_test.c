...
void pyc_test_pdePycProc() {
    ...
    printf("]\n");
    printf("%s %d\n", "mat[2][1]", mat[(2) * (mat_w) + (1)]);
    printf("%s %d\n", "loop_test", loop_test(12));
    printf("%s %d\n", "sum_array", sum_array(arr, arr_len));
    printf("%s %d\n", "sum2d", sum2d(mat, mat_h, mat_w));
    printf("%s %d\n", "mix_math", mix_math(9, 4));
    printf("%s %d\n", "bit_mix", bit_mix(b, 0xF0));
    printf("%s %d\n", "bool_ops", bool_ops(a, 0));
    Point p = (Point){3, 7};
    p.x = (p.x + 1);
    printf("%s %d\n", "point_sum", point_sum(p));
    printf("%s %u\n", "lcg_step", lcg_step(123456));
    printf("%s %d\n", "local_arr", local_arr());
    printf("%s %d\n", "mem_ops", mem_ops());
    return;
}
