...
class MeasureWorker : public QObject
{
    Q_OBJECT
public:
    explicit MeasureWorker(QObject *parent = nullptr) : QObject(parent), m_isBusy(false) {}
    // 设置标定矩阵
    void setHomography(const cv::Mat &h) { m_homographyMat = h.clone(); }
public slots:
    // -----------------------------------------------------------------------
    // doProcess: 双通道竞争模式 (Double-Channel Competition)
    // 同时跑 "二值化" 和 "反转二值化"，自动择优，解决一切光照和背景问题
    // -----------------------------------------------------------------------
    void doProcess(cv::Mat smallMat, double scale) {
        if (m_isBusy) return;
        m_isBusy = true;
        ...
        // 2. 双通道准备：一个找白物体(Normal)，一个找黑物体(Inv)
        cv::Mat binNormal, binInv;

        // 通道 A: 正常二值化 (找黑背景上的亮物体)
        double threshVal1 = cv::threshold(gray, binNormal, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);
        // [保护] 防止全黑背景下的噪点
        if (threshVal1 < 30) cv::threshold(gray, binNormal, 30, 255, cv::THRESH_BINARY);

        // 通道 B: 反转二值化 (找亮背景上的黑物体)
        // 注意：这里不用再做一次 OTSU，直接取反 binNormal 即可，效率最高
        cv::bitwise_not(binNormal, binInv);

        // 3. 定义筛选变量
        double maxArea = 0;
        std::vector<cv::Point> bestContour;
        double minAreaThreshold = 1000 * scale * scale; // 最小面积过滤
        int w = gray.cols;
        int h = gray.rows;
        int margin = 5; // 边缘容差

        // 定义一个 Lambda 函数来处理轮廓查找和筛选，避免写两遍重复代码
        auto processContours = [&](const cv::Mat& binImg) {
            ...
            for (const auto &cnt : contours) {
                ...
                if (box.x <= margin || box.y <= margin ||
                    (box.x + box.width) >= (w - margin) ||
                    (box.y + box.height) >= (h - margin))
                {
                    continue;
                }
        ...
        // 4. 让两个通道竞争
        processContours(binNormal); // 看看“正常模式”有没有好结果
        processContours(binInv);    // 看看“反转模式”有没有好结果

        // 5. 输出赢家结果
        if (!bestContour.empty()) {

            cv::RotatedRect rRect = cv::minAreaRect(bestContour);
            cv::Point2f ptsSmall[4];
            rRect.points(ptsSmall);

            QVector<QPointF> resultPts;
            std::vector<cv::Point2f> cvPtsBig;
            for(int i=0; i<4; i++) {
                cv::Point2f ptBig = ptsSmall[i] * (1.0 / scale);
                resultPts.append(QPointF(ptBig.x, ptBig.y));
                cvPtsBig.push_back(ptBig);
            }

            if (!m_homographyMat.empty()) {
                std::vector<cv::Point2f> worldPts;
                cv::perspectiveTransform(cvPtsBig, worldPts, m_homographyMat);

                double sideA = cv::norm(worldPts[0] - worldPts[1]);
                double sideB = cv::norm(worldPts[1] - worldPts[2]);

                emit resultReady(resultPts, std::max(sideA, sideB), std::min(sideA, sideB));
            }
        } else {
             emit resultReady(QVector<QPointF>(), 0, 0);
        }
...
signals:
    // 信号：通知 UI 更新显示
    void resultReady(QVector<QPointF> pts, double len, double wid);

private:
    std::atomic<bool> m_isBusy; // 原子变量，线程安全标志
    cv::Mat m_homographyMat;
};
...
private:
    ...
    void updateCircleIfoLables();      //根据m_circleCenterOverlay、m_circleRadiusOverlay刷新LableCircleCenter1-9,LableCircleRadius1-9

    void promptForWorldCoordinate(int index);//用户一个个输入世界坐标
    /**
     * @brief 导入世界坐标点辅助函数
     * @details 弹出文件对话框，读取用户选择的 CSV/TXT 文件，并解析出九个世界坐标点。
     * @return 导入成功返回 true，否则返回 false。
     */
    bool importWorldCoordinates();
...
signals:
    void sigNewFrame(const QImage &img);
    // 信号：主线程 -> 子线程
    void startWorkerProcess(cv::Mat smallMat, double scale);
...
private slots:
    ...
    void on_bnFindCircle_clicked();   //找圆心按钮

    void onNewFrame(const QImage &img);  // 用于更新 QGraphicsView

    void on_bnFindBox_clicked();   // 找盒子四点

    void on_bnRectify_clicked();   //畸变矫正


    void on_bnStartCalib9_clicked();   // 开始/进入九点标定
    void on_bnStartCalculate_clicked();   // 开始计算转换矩阵
    void on_bnSaveCalib9_clicked();    // 保存九点标定结果
    //动态测量按钮槽函数
    void on_bnDynamicMeasure_clicked();
...
protected:
    bool eventFilter(QObject *watched, QEvent *event) override; // 显示鼠标悬停处行列值、灰度值
...
private:
    ...
    QVector<float> m_calibRadii;           // 对应的 9 个标定圆的半径
    /**
     * @brief 单应性矩阵 (Homography Matrix)
     * @details 3x3 的矩阵，用于将图像坐标 [u, v, 1]^T 转换为世界坐标 [X, Y, 1]^T。
     * OpenCV Mat 类型，存储 double 精度数据。
     */
    cv::Mat m_H;

    /**
     * @brief 单应性矩阵状态标记
     * @details 标记 m_H 是否已成功计算且可用。
     */
    bool m_hasH = false;
    ...
    QVector<cv::Point2f> m_circleCenterOverlay;    // 存储多个圆心坐标
    QVector<float> m_circleRadiusOverlay;          // 存储多个半径

//    cv::Point2f m_circleCenterOverlay;//原来单个圆的圆心坐标
//    float       m_circleRadiusOverlay = 0.0f;//原来单个圆的半径
    ...
    // 动态测量相关变量
    bool m_isDynamicMeasuring;       // 是否处于动态测量模式
    cv::Mat m_homographyMat;         // 存储 3x3 透视变换矩阵

    // 辅助函数：加载矩阵文件
    bool loadHomographyMatrix(const QString &filePath);

    // 核心处理函数：在图像上进行测量并画图
    // 返回值用于判断是否检测到了物体，方便更新UI
    void processDynamicMeasure(cv::Mat &displayMat);

    // 辅助函数：像素点转物理坐标点
    cv::Point2f pixelToWorld(const cv::Point2f &pixelPt);

    // 多线程相关变量
    QThread* m_workerThread;
    MeasureWorker* m_worker;

    // 存储最新的测量结果（用于 UI 绘制）
    QVector<QPointF> m_lastMeasurePts; // 矩形4点
    double m_lastLen = 0.0;
    double m_lastWid = 0.0;
    // --- 相机内参标定相关变量 ---

    // 棋盘格的内部角点数量 (例如：如果棋盘是10x7个方块，那么角点是 9x6)
    const cv::Size m_boardSize = cv::Size(6, 6);
    // 棋盘格每个方块的实际边长 (单位：毫米 mm)
    const float m_squareSize = 10.0f;
    // 存储所有图像的角点 (2D点)
    std::vector<std::vector<cv::Point2f>> m_imagePointsSeq;
    // 存储对应的世界坐标点 (3D点)
    std::vector<std::vector<cv::Point3f>> m_objectPointsSeq;

    // 标定结果
    cv::Mat m_cameraMatrix;   // 内参矩阵
    cv::Mat m_distCoeffs;     // 畸变系数
    bool m_isCalibrated = false; // 是否标定完成

    // 辅助函数：生成通过 calibrateCamera 所需的世界坐标点
