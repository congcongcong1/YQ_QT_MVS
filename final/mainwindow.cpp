...
static void sortRectPoints(std::vector<cv::Point2f> &pts)
{
    ...
    // 但在旋转矩形中，通常定义：先找最上面的点，如果有两个，取左边那个
    // 为保持和原有逻辑一致（P1:左上, P2:右上, P3:右下, P4:左下），需要调整顺序
...
void MainWindow::onNewFrame(const QImage &img)
{
    ...
    if (!pix.isNull())
    {
        ...
        if (m_isDynamicMeasuring && !m_lastMeasurePts.isEmpty()) {
            QPen pen(Qt::green); pen.setWidth(10); painter.setPen(pen); painter.setBrush(Qt::NoBrush);
        ...
        if (m_hasCircleOverlay && !m_circleRadiusOverlay.isEmpty()) {
             QPen penCircle(Qt::red); penCircle.setWidth(12); painter.setPen(penCircle); painter.setBrush(Qt::NoBrush);
        ...
        if (m_hasQuad && m_lastQuad.size() == 4) {
            QPen penSel(Qt::red); penSel.setWidth(9); painter.setPen(penSel); painter.setBrush(Qt::NoBrush);
            QPolygonF poly;
        ...
        if (m_hasRoi) {
            ...
            if (r.width()>1 && r.height()>1) {
                QPen p(Qt::green); p.setWidth(9); p.setStyle(Qt::DashLine); painter.setPen(p);
                painter.drawRect(r);
            }
        }
        if (m_isMeasuring) {
            ...
            // 3. 【新增】设置大字体
            QPen penText(Qt::yellow);
            painter.setPen(penText);

            QFont font = painter.font();
            font.setPixelSize(60);       // <--- 这里修改字体大小，60 适合大分辨率图片，可根据需要调整为 40-100
            font.setBold(true);          // <--- 加粗
            painter.setFont(font);

            // 4. 绘制文字 (居中显示)
            // 为了让文字不压在线上，稍微偏移一点 (y - 20)
            QPointF center((m_measureStart.x + m_measureEnd.x) / 2, (m_measureStart.y + m_measureEnd.y) / 2);
            painter.drawText(center + QPointF(0, -20), QString::number(dist, 'f', 1));
        }

        // ==========================================
        // [修改] FPS + 平均灰度 (MeanGray) 计算区域
        // ==========================================
        m_frameCountForFps++;
        if (m_fpsTimer.elapsed() >= 1000) {
            ...
            if (!m_displayImage.isNull()) {
                ...
                // 构造 OpenCV Mat 头 (不拷贝数据，速度快)
                // 注意：必须确保 m_displayImage 格式匹配
                if (m_displayImage.format() == QImage::Format_Grayscale8) {
                    cv::Mat mat(m_displayImage.height(), m_displayImage.width(), CV_8UC1, (void*)m_displayImage.constBits(), m_displayImage.bytesPerLine());
                    cv::Scalar s = cv::mean(mat);
                    meanVal = s[0];
                }
                else if (m_displayImage.format() == QImage::Format_RGB888) {
                    cv::Mat mat(m_displayImage.height(), m_displayImage.width(), CV_8UC3, (void*)m_displayImage.constBits(), m_displayImage.bytesPerLine());
                    cv::Scalar s = cv::mean(mat);
                    // BGR/RGB 转灰度估算公式
                    meanVal = (s[0] + s[1] + s[2]) / 3.0;
                }

                // 更新 UI
                ui->labelMeanGray->setText(QString::number(meanVal, 'f', 1));
            }
        }
    }
