...
int MainWindow::GetTriggerMode()
{
    ...
    for (int i = 0; i < stEnumTriggerModeValue.nSupportedNum; i++)
    {
        ...
        {
            m_nTriggerMode = i;
            ui->SelchangeTriggerswitchCombo->setCurrentIndex(m_nTriggerMode);
        }
    }
...
int MainWindow::SaveImage(MV_SAVE_IAMGE_TYPE enSaveImageType)
{
    ...
    {
    case MV_Image_Bmp:
        defaultName = QString("Image_%1.bmp").arg(timeStr);
        filter = tr("BMP Images (*.bmp)");
        break;

    case MV_Image_Jpeg:
        defaultName = QString("Image_%1.jpg").arg(timeStr);
        filter = tr("JPEG Images (*.jpg *.jpeg)");
        break;

    case MV_Image_Tif:
        defaultName = QString("Image_%1.tif").arg(timeStr);
        filter = tr("TIFF Images (*.tif *.tiff)");
        break;

    case MV_Image_Png:
        defaultName = QString("Image_%1.png").arg(timeStr);
    ...
    // 设置初始路径：上次保存目录下的默认文件名
    QString initialPath = defaultName;
    if (!m_saveDir.isEmpty())
    {
        initialPath = QDir(m_saveDir).filePath(defaultName);
    }
    ...
    // 更新上次保存目录
    m_saveDir = QFileInfo(filePath).absolutePath();
    ...
    // 设置 JPG/PNG 质量参数
    if (enSaveImageType == MV_Image_Jpeg)
    {
        stSaveFileParam.nQuality = 80;   // JPG 质量，(50,99]
    }
    else if (enSaveImageType == MV_Image_Png)
    {
...
{
    // ==================== 1. 初始检查和错误处理 ====================
       // 目的：确保有有效的图像数据可以处理，避免后续操作出现异常
    QMutexLocker locker(&m_frameMutex);//加锁保护当前帧数据，防止多线程冲突
    if (!m_hasFrame) {
        QMessageBox::information(this, tr("提示"), tr("无图像"));
        return;
    }

    int imgW = m_frameInfo.nWidth;//获取图像宽度
    int imgH = m_frameInfo.nHeight;//获取图像高度

    //检测图像尺寸有效性，避免处理过小的图像
    if(imgW < 50 || imgH < 50)
    {
        QMessageBox::warning(this,tr("错误"),tr("图像尺寸太小，无法处理"));
        return ;
    }
    ...
    {
        QMessageBox::warning(this,tr("错误"), tr("不支持的像素格式"));
        return ;
    }
//    else
//    {
...
//            cv::cvtColor(src, gray, cv::COLOR_RGB2GRAY);
//    }
   // locker.unlock();//解锁，后续操作不在需要访问共享数据

    //检查灰度图是否创建成功
    if (gray.empty())
    {
        QMessageBox::warning(this,tr("错误"),tr("灰度图转换失败"));
        return;
    }

    // ==================== 2. ROI处理 ====================
       // 目的：确定处理区域，提高处理效率和准确性
    cv::Rect roiRect(0, 0, imgW, imgH);

    //如果用户设置了ROI，则使用用户定义的ROI
    if (m_hasRoi)
    {
        QRectF r = currentRoiRect().normalized();//获取归一化的ROI矩形
        int x = std::max(0, (int)std::floor(r.left()));//计算ROI左上角X的坐标
        int y = std::max(0, (int)std::floor(r.top()));//计算ROI左上角Y的坐标
        int w = std::min(imgW - x, (int)std::ceil(r.width()));//计算ROI宽度，确保不越界
        int h = std::min(imgH - y, (int)std::ceil(r.height()));//计算ROI高度，确保不越界
        //检查ROI尺寸是否合理
        if (w <= 5 || h <= 5) {
            QMessageBox::information(this, tr("提示"), tr("ROI 区域太小"));
            return;
    ...
    // ==================== 3. 自适应二值化 ====================
    // 目的：将灰度图转换为二值图，使用多种方法提高对不同光照条件的适应性
    // 3) 高斯模糊
    cv::Mat blurred;
    cv::GaussianBlur(roiGray, blurred, cv::Size(9, 9), 2, 2);

    // 方法1: 自适应阈值 - 对光照不均的图像效果好
        cv::Mat adaptiveBin;
        cv::adaptiveThreshold(blurred, adaptiveBin, 255,
                             cv::ADAPTIVE_THRESH_GAUSSIAN_C,  // 使用高斯加权平均
                             cv::THRESH_BINARY, 21, 10);      // 块大小21，常数10

    // 方法2: Otsu阈值 - 自动计算最佳全局阈值
    cv::Mat otsuBin;
    double otsuThresh = cv::threshold(blurred, otsuBin, 0, 255,
                                             cv::THRESH_BINARY | cv::THRESH_OTSU);

    // 方法3: 智能阈值选择 - 基于图像特性选择二值化方向
    cv::Mat smartBin;
       bool useBrightObject = detectBrightObject(blurred);  // 检测图像中主要是亮物体还是暗物体
       if (useBrightObject) {
           // 亮物体：使用较低阈值保留更多细节
           cv::threshold(blurred, smartBin, otsuThresh * 0.7, 255, cv::THRESH_BINARY);
       } else {
           // 暗物体：使用较高阈值减少噪声
           cv::threshold(blurred, smartBin, otsuThresh * 1.3, 255, cv::THRESH_BINARY_INV);
       }

     // ==================== 4. 防粘连处理 ====================
          // 目的：通过形态学操作分离粘连的物体，同时保持形状完整性

    std::vector<cv::Mat> binaryMats = {adaptiveBin,otsuBin,smartBin};//三种二值图
    std::vector<cv::Point2f> allCandidates;//存储所有检测到的候选四边形
    std::vector<cv::Point2f> bestCandidate;//存储最佳候选四边形
    double bestScore = -1.0;//最佳评分，初始值为负值

    //对每种二值化方法分别处理
    for(size_t binIdx = 0; binIdx < binaryMats.size();++binIdx)
    {
        cv::Mat binary = binaryMats[binIdx].clone();//复制二值图，避免修改原图

        //自适应形态学核大小 - 根据图像尺寸动态调整
        int morphSize = calculateAdaptiveMorphSize(roiRect.width,roiRect.height);
        ...
        for (const auto& contour :contours)
        {
            ...
            double roiArea = roiRect.area();
            if (!validateArea(area, roiArea)) continue;

            //周长筛选 - 排除过于简单的轮廓 （可能是噪声）
            ...
            if (rectArea < 1.0) continue ;//避免除零错误

            double solidity = area / rectArea;// 实心度 = 轮廓面积/外接矩形面积
            if (solidity < 0.6) continue;

            // ==================== 6. 双重角点获取 ====================
                        // 目的：通过两种方法获取四边形的四个角点，提高成功率

            std::vector<cv::Point2f> quad;//存储四边形四个角点
            bool quadFound = false; //标记是否成功获取四边形


            //  多边形近似 - 优先使用，能更好保持原始形状
            std::vector<cv::Point> approx;
            double epsilon = 0.02 * perimeter;  // 动态epsilon，基于周长调整近似精度
            cv::approxPolyDP(contour, approx, epsilon, true);  // 多边形近似
            ...
            if (approx.size() == 4 && cv::isContourConvex(approx))
            {
                for (const auto& p : approx)
                  {
                        quad.push_back(cv::Point2f(p.x, p.y));  // 转换坐标类型
                  }
                  quadFound = true;  // 标记成功获取
             }
            // 方法B: 最小外接矩形 - 备用方法，保证总能得到四边形
            if (!quadFound)
            {
                cv::Point2f rectPoints[4];
                minRect.points(rectPoints);  // 获取旋转矩形的四个顶点
                for (int i = 0; i < 4; ++i)
                {
                    quad.push_back(rectPoints[i]);  // 添加到四边形点集
                }
                    quadFound = true;  // 标记成功获取
             }

             if (!quadFound) continue;  // 两种方法都失败，跳过该轮廓

             // ==================== 7. 四边形验证 ====================
                        // 目的：确保获取的是有效的四边形，排除畸形四边形

             if (!validateQuadrilateral(quad)) continue;

             // ==================== 8. 亚像素精确定位 ====================
                         // 目的：将角点位置精确到亚像素级别，提高定位精度

             refineQuadCorners(roiGray, quad);
             ...
             //确保一致的顶点顺序 （左上，右上，左下，右下）
             sortRectPoints(quad);

             // ==================== 9. 智能评分 ====================
                         // 目的：为每个候选四边形评分，选择最可能是盒子的那个

             double score = calculateBoxScore(quad, roiCenter, area, solidity, binIdx);

             //保存候选到全局列表 （用于显示所有检测结果）
             allCandidates.insert(allCandidates.end(), quad.begin(), quad.end());
    ...
    // ==================== 10. 结果处理和错误处理 ====================
       ...
       // 更新UI显示最佳盒子的四个角点坐标
       updateBoxUI(bestCandidate);

       // 更新叠加显示（最佳候选和所有候选）
       updateBoxOverlay(bestCandidate, allCandidates);
 ...
 * @brief 检测图像中是否主要为亮物体
 * @param gray 输入灰度图像
 * @return true-亮物体，false-暗物体
 * @details 通过比较中心区域和全局平均灰度来判断
 */
bool MainWindow::detectBrightObject(const cv::Mat& gray)
{
    // 输入检查：确保图像非空且有效
    if (gray.empty() || gray.cols <= 0 || gray.rows <= 0) {
        return false;  // 无效图像默认返回 false
    }

    cv::Scalar meanVal = cv::mean(gray);// 计算全局平均灰度
    cv::Mat blurred;
    cv::GaussianBlur(gray, blurred, cv::Size(15,15), 2.0);//模糊处理减少噪声影响

    //计算中心区域的平均灰度（中心区域大小为图像的1/4）
    int size = std::min(gray.cols, gray.rows) / 4;
    
    // 边界检查：确保 size 有效
    if (size <= 0) {
        return meanVal[0] > 128;  // 图像太小，使用全局平均灰度判断
    }
    ...
    // 修正：先计算坐标，然后确保不越界
    int rectX = std::max(0, centerX - size / 2);
    int rectY = std::max(0, centerY - size / 2);
    int rectWidth = std::min(size, gray.cols - rectX);
    int rectHeight = std::min(size, gray.rows - rectY);
    
    // 再次检查：确保区域有效
    if (rectWidth <= 0 || rectHeight <= 0) {
        return meanVal[0] > 128;  // 无法创建有效中心区域，使用全局判断
    }
    
    cv::Rect centerRect(rectX, rectY, rectWidth, rectHeight);
    
    // 使用中心区域平均灰度与全局平均灰度比较
    cv::Scalar centerMean = cv::mean(gray(centerRect));
    return centerMean[0] > meanVal[0];  // 中心比整体亮说明是亮物体
}
 ...
 * @param height 图像高度
 * @return 形态学操作核大小
 * @details 根据图像尺寸动态计算合适的核大小
 */
int MainWindow::calculateAdaptiveMorphSize(int width, int height)
{
    int minDim = std::min(width, height);  // 取宽度和高度中的较小值
       int size = minDim / 40;                // 基于图像尺寸的比例计算（1/40）
       size = std::max(3, std::min(size, 15)); // 限制核大小在3-15像素之间
       return size;
}
 ...
 * @brief 验证面积是否合理
 * @param area 轮廓面积
 * @param roiArea ROI区域总面积
 * @return true-面积合理，false-面积不合理
 * @details 基于ROI面积的相对阈值，排除过大或过小的轮廓
 */
bool MainWindow::validateArea(double area, double& roiArea)
{
    double minArea = roiArea * 0.005;  // 最小面积：ROI的0.5%
        double maxArea = roiArea * 0.8;    // 最大面积：ROI的80%
 ...
 * @brief 严格的四边形验证
 * @param quad 四边形四个角点
 * @return true-有效四边形，false-无效四边形
 * @details 从凸性、角度、长宽比、面积等多个维度验证四边形有效性
 */
bool MainWindow::validateQuadrilateral(const std::vector<cv::Point2f>& quad)
{
    if (quad.size() != 4) return false;  // 必须正好4个点

        // 1. 凸性检查 - 确保四边形是凸的
        std::vector<cv::Point2f> hull;
        cv::convexHull(quad, hull);  // 计算凸包
        if (hull.size() != 4) return false;  // 凸包点数不为4说明不是凸四边形

        // 2. 角度检查 - 避免过于尖锐或平坦的角度
        const double minAngle = 45.0;  // 最小角度45度
        const double maxAngle = 135.0; // 最大角度135度

        for (int i = 0; i < 4; ++i) {
            cv::Point2f a = quad[i];
            cv::Point2f b = quad[(i + 1) % 4];  // 下一个点
            cv::Point2f c = quad[(i + 2) % 4];  // 下下个点

            cv::Point2f ab = b - a;  // 向量AB
            cv::Point2f bc = c - b;  // 向量BC
            ...
            // 计算角度（弧度转角度）
            double angle = std::atan2(std::abs(crossProduct), dotProduct) * 180.0 / CV_PI;
        ...
        // 3. 长宽比检查 - 避免过于细长的形状
        double width1 = cv::norm(quad[0] - quad[1]);  // 上边长度
        double width2 = cv::norm(quad[2] - quad[3]);  // 下边长度
        double height1 = cv::norm(quad[0] - quad[2]); // 左边长度
        double height2 = cv::norm(quad[1] - quad[3]); // 右边长度

        double avgWidth = (width1 + width2) * 0.5;    // 平均宽度
        double avgHeight = (height1 + height2) * 0.5; // 平均高度

        if (avgWidth < 10 || avgHeight < 10) return false; // 尺寸太小

        double aspectRatio = std::max(avgWidth, avgHeight) / std::min(avgWidth, avgHeight);
        if (aspectRatio > 10.0) return false; // 长宽比超过10:1认为过于细长

        // 4. 面积有效性检查
        std::vector<cv::Point2f> contour(quad.begin(), quad.end());
        double quadArea = cv::contourArea(contour);  // 计算四边形面积
        if (quadArea < 100) return false; // 面积太小，可能是噪声
 ...
 * @brief 亚像素角点精化
 * @param gray 输入灰度图像
 * @param quad 四边形角点（输入输出参数）
 * @details 使用OpenCV的角点亚像素优化算法提高定位精度
 */
void MainWindow::refineQuadCorners(const cv::Mat& gray, std::vector<cv::Point2f>& quad)
{
    // 检查边界条件 - 角点不能太靠近图像边界
       for (auto& point : quad) {
           if (point.x < 3 || point.y < 3 ||
               point.x > gray.cols - 4 || point.y > gray.rows - 4) {
               return; // 太靠近边界，不进行亚像素优化（避免越界）
           }
       }
       ...
       try {
           // 执行亚像素角点优化
           cv::cornerSubPix(gray, quad, cv::Size(5, 5), cv::Size(-1, -1), criteria);
       } catch (const cv::Exception& e) {
           // 亚像素优化失败，使用原始点（安全处理）
           qDebug() << "亚像素优化失败:" << e.what();
       }
}
 ...
 * @brief 智能评分机制
 * @param quad 四边形角点
 * @param roiCenter ROI中心点
 * @param area 轮廓面积
 * @param solidity 实心度
 * @param methodIdx 二值化方法索引
 * @return 综合评分（0-1之间，越高越好）
 * @details 综合考虑形状质量、接近正方形程度、位置等因素
 */
double MainWindow::calculateBoxScore(const std::vector<cv::Point2f>& quad,
                         const cv::Point2f& roiCenter,
                         double area, double solidity, int methodIdx)
{
    // 1. 形状质量评分 (40%)
        double shapeScore = solidity * 0.7;  // 实心度占形状评分的70%

        // 计算四边形四条边的长度
        std::vector<double> sides = {
            cv::norm(quad[0] - quad[1]),  // 上边
            cv::norm(quad[1] - quad[2]),  // 右边
            cv::norm(quad[2] - quad[3]),  // 下边
            cv::norm(quad[3] - quad[0])   // 左边
        };

        // 计算平均边长和边长方差
        double avgSide = std::accumulate(sides.begin(), sides.end(), 0.0) / 4.0;
        double sideVariance = 0.0;
        for (double side : sides) {
            sideVariance += std::abs(side - avgSide);  // 计算绝对偏差
        }
        sideVariance /= (4 * avgSide);  // 归一化方差

        double regularityScore = 1.0 - std::min(sideVariance, 1.0);  // 规则度评分
        shapeScore += regularityScore * 0.3;  // 规则度占形状评分的30%

        // 2. 接近正方形程度评分 (30%)
        double minSide = *std::min_element(sides.begin(), sides.end());  // 最短边
        double maxSide = *std::max_element(sides.begin(), sides.end());  // 最长边
        double squareness = minSide / maxSide;  // 正方形程度（越接近1越像正方形）
        double squareScore = squareness;

        // 3. 位置评分 (20%) - 距离中心越近分数越高
        cv::Point2f quadCenter(0, 0);
        for (const auto& p : quad) quadCenter += p;  // 计算四边形中心
        quadCenter *= 0.25;

        double maxDistance = std::max(roiCenter.x, roiCenter.y);  // 最大可能距离
        double distance = cv::norm(quadCenter - roiCenter);       // 实际距离
        double positionScore = 1.0 - std::min(distance / maxDistance, 1.0);

        // 4. 方法权重 (10%) - 不同二值化方法的可信度不同
        double methodWeight = 1.0;
        if (methodIdx == 0) methodWeight = 1.0;      // 自适应阈值 - 标准权重
        else if (methodIdx == 1) methodWeight = 0.9; // Otsu阈值 - 稍低权重
        else if (methodIdx == 2) methodWeight = 1.1; // 智能阈值 - 稍高权重

        // 综合评分：加权求和
        double finalScore = (shapeScore * 0.4 + squareScore * 0.3 +
                            positionScore * 0.2) * methodWeight;
...
/**
 * @brief 更新UI显示
 * @param bestCandidate 最佳盒子的四个角点
 * @details 在界面标签上显示四个角点的坐标
 */
void MainWindow::updateBoxUI(const std::vector<cv::Point2f>& bestCandidate)
{
 ...
 * @brief 更新叠加显示
 * @param bestCandidate 最佳盒子的四个角点
 * @param allCandidates 所有检测到的候选四边形
 * @details 更新内部状态，用于在图像上绘制检测结果
 */
void MainWindow::updateBoxOverlay(const std::vector<cv::Point2f>& bestCandidate,
                      const std::vector<cv::Point2f>& allCandidates)
{
    if (bestCandidate.size() == 4) {
           // 分别更新四个角点的坐标显示
           ui->labelBoxP1->setText(QString("(%1, %2)")
               .arg(bestCandidate[0].x, 0, 'f', 1)  // P1点x坐标，保留1位小数
               .arg(bestCandidate[0].y, 0, 'f', 1)); // P1点y坐标
           ui->labelBoxP2->setText(QString("(%1, %2)")
               .arg(bestCandidate[1].x, 0, 'f', 1)  // P2点x坐标
               .arg(bestCandidate[1].y, 0, 'f', 1)); // P2点y坐标
           ui->labelBoxP3->setText(QString("(%1, %2)")
               .arg(bestCandidate[2].x, 0, 'f', 1)  // P3点x坐标
               .arg(bestCandidate[2].y, 0, 'f', 1)); // P3点y坐标
           ui->labelBoxP4->setText(QString("(%1, %2)")
               .arg(bestCandidate[3].x, 0, 'f', 1)  // P4点x坐标
               .arg(bestCandidate[3].y, 0, 'f', 1)); // P4点y坐标
       }
}
...
void MainWindow::onNewFrame(const QImage &img)
{
    ...
    m_pixItem->setPixmap(pix);
    m_scene->setSceneRect(pix.rect());
    if (m_isFirstFrameAfterStart)
    {
        ui->DisplayWidget->resetTransform();
        ui->DisplayWidget->fitInView(m_scene->sceneRect(), Qt::KeepAspectRatio);
        m_isFirstFrameAfterStart = false;
    }
}
