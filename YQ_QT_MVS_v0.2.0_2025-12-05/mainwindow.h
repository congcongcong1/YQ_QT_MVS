...
private:
    ...
    //用于找盒子四点
    bool detectBrightObject(const cv::Mat& gray);//检测图像中是否主要为亮物体
    int calculateAdaptiveMorphSize(int width, int height); //计算自适应形态学核大小
    bool validateArea(double area, double& roiArea);//验证面积是否合理
    bool validateQuadrilateral(const std::vector<cv::Point2f>& quad);//严格的四边形验证
    void refineQuadCorners(const cv::Mat& gray, std::vector<cv::Point2f>& quad);//亚像素角点精化
    double calculateBoxScore(const std::vector<cv::Point2f>& quad,
                             const cv::Point2f& roiCenter,
                             double area, double solidity, int methodIdx);//智能评分机制
    void updateBoxUI(const std::vector<cv::Point2f>& bestCandidate);//更新UI显示
    void updateBoxOverlay(const std::vector<cv::Point2f>& bestCandidate,
                          const std::vector<cv::Point2f>& allCandidates);//更新叠加显示
...
private:
    ...
    bool                    m_isFirstFrameAfterStart = false;  // 开始采集后的第一帧需自适应
