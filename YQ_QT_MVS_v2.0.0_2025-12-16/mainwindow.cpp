...
// 开启/关闭实时测量
void MainWindow::on_chkRealTimeMeasure_clicked(bool checked)
{
    m_isRealTimeMeasure = checked;
    // 如果没有输入比例，默认给一个值防止除以0，或者提示用户
    if (m_pixelsPerCm <= 0.1f) {
        m_pixelsPerCm = 331.42f; // 默认假设
        ui->linePixelsPerCm->setText("331.42");
    }
}

// 更新标定比例
void MainWindow::on_linePixelsPerCm_editingFinished()
{
    float val = ui->linePixelsPerCm->text().toFloat();
    if (val > 0.1f) {
        m_pixelsPerCm = val;
    }
}
...
/**
 * @brief 实时测量核心逻辑
 * @param painter Qt画笔，用于在图上画框和文字
 * @param img 当前显示的图像
 */
/**
 * @brief 融合了“重度防粘连”算法的实时测量函数
 * @details 移植了 on_bnFindBox_clicked 的核心逻辑：高斯模糊 -> 双重阈值 -> 大核腐蚀去光晕 -> 亚像素优化
 */
void MainWindow::processAndDrawMeasurements(QPainter &painter, const QImage &img)
{
    // === 🔴 1. 强制给一个默认值，防止因为没按回车导致不显示 ===
    if (m_pixelsPerCm <= 0.1f) {
        m_pixelsPerCm = 331.42f;
        qDebug() << "Warning: No calibration! Using default" << m_pixelsPerCm;
    }

    if (img.isNull()) return;

    // 1. 格式转换：QImage -> cv::Mat
    cv::Mat gray;
    if (img.format() == QImage::Format_Grayscale8) {
        gray = cv::Mat(img.height(), img.width(), CV_8UC1, (void*)img.constBits(), img.bytesPerLine());
    } else {
        QImage tmp = img.convertToFormat(QImage::Format_Grayscale8);
        gray = cv::Mat(tmp.height(), tmp.width(), CV_8UC1, (void*)tmp.constBits(), tmp.bytesPerLine()).clone();
    }
    ...
    // 4. 定义处理 Lambda (已修改：只画最大的一个轮廓)
    auto processAndDraw = [&](cv::Mat mask, QColor boxColor)
    {
        // --- 核心：重度腐蚀 + 恢复 ---
        cv::erode(mask,  mask, kernelHeavy,   cv::Point(-1,-1), 2); // 腐蚀 2 次
        cv::dilate(mask, mask, kernelRestore, cv::Point(-1,-1), 2); // 膨胀 2 次
        ...
        // --- 筛选逻辑 ---
        double area = cv::contourArea(cnt);
        // 阈值设大一点 (50000)，防止把大的反光斑点当成物体
        if (area < 50000) return;

        cv::RotatedRect rRect = cv::minAreaRect(cnt);
        double rectArea = rRect.size.width * rRect.size.height;
        if (rectArea <= 1.0) return;

        double solidity = area / rectArea;
        // 只要比较实心的矩形 (防止检测到奇形怪状的光晕)
        if (solidity < 0.6) return;
        ...
        std::vector<cv::Point2f> quad;
        if (approx.size() == 4 && cv::isContourConvex(approx)) {
            for (auto &p : approx) quad.push_back(cv::Point2f(p.x, p.y));
        } else {
            cv::Point2f pts[4];
            rRect.points(pts);
            for (int i = 0; i < 4; ++i) quad.push_back(pts[i]);
        }
        ...
        bool inBound = true;
        for(auto &p : quad) {
            if(p.x < 5 || p.y < 5 || p.x > gray.cols-5 || p.y > gray.rows-5) { inBound = false; break;}
        }
        if(inBound) {
            cv::TermCriteria criteria(cv::TermCriteria::EPS + cv::TermCriteria::COUNT, 5, 0.1);
        ...
        */
        ...
        // --- 绘制图形 ---
        QPolygonF poly;
        for(const auto &p : quad) poly << QPointF(p.x, p.y);

        QPen penBox(boxColor);
        penBox.setWidth(5);
        ...
        // --- 计算并绘制尺寸 ---
        // 使用 0-1 和 1-2 两条边计算长宽
        // 注意：由于没有使用 sortRectPoints，这里的 0-1 可能是长边也可能是短边，但测量值是对的
        float dx1 = quad[0].x - quad[1].x; float dy1 = quad[0].y - quad[1].y;
        float len1 = std::sqrt(dx1*dx1 + dy1*dy1);

        float dx2 = quad[1].x - quad[2].x; float dy2 = quad[1].y - quad[2].y;
        float len2 = std::sqrt(dx2*dx2 + dy2*dy2);

        float cm1 = len1 / m_pixelsPerCm;
        float cm2 = len2 / m_pixelsPerCm;

        // 找中点画文字
        QPointF mid1 = (QPointF(quad[0].x, quad[0].y) + QPointF(quad[1].x, quad[1].y)) / 2;
        QPointF mid2 = (QPointF(quad[1].x, quad[1].y) + QPointF(quad[2].x, quad[2].y)) / 2;

        painter.setPen(Qt::red);
        painter.drawText(mid1, QString::number(cm1, 'f', 1) + "cm");

        painter.setPen(Qt::green);
        painter.drawText(mid2, QString::number(cm2, 'f', 1) + "cm");
    };

    // 5. 执行检测 (针对白色亮物体)
    cv::Mat binBright;
    // 使用 OTSU 自动阈值
    cv::threshold(blurred, binBright, 0, 255, cv::THRESH_BINARY | cv::THRESH_OTSU);

    // 调用处理函数，画蓝色框
    processAndDraw(binBright, QColor(0, 0, 255));
}
void MainWindow::onNewFrame(const QImage &img)
{
    if (img.isNull()) return;
    ...
    if (!pix.isNull())
    {
        ...
        // ✨ 新增：实时测量调用
        // ==========================================
        if (m_isRealTimeMeasure) {
             processAndDrawMeasurements(painter, m_displayImage);
        }
//        processAndDrawMeasurements(painter, m_displayImage);
        painter.end();
    }
